...
To illustrate the two methods, imagine we're developing an application that needs to display error messages in many different components. All of the error messages should be red and bold. Some error messages should also use a large font.
...
Now imagine if there are 100 avatars on the page! See [Michael Dougall's blog post](https://douges.dev/blog/taming-the-beast-that-is-css-in-js) for a live example of this issue — it's under the Performance heading.
...
TypeScript users: you'll have to use a type assertion like `style={{ ['--background-style' as any]: imageUrl }}` as explained [here](https://stackoverflow.com/a/52013197/752601).

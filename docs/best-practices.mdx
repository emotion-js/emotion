...
Now imagine if there are 100 avatars on the page! See [Michael Dougall's blog post] for a live example of this issue — it's under the Performance heading.
...
### Use `@emotion/react` or `@emotion/styled`, but not both

While the css prop and `styled` can peacefully coexist, it's better to pick one approach and use it consistently across your codebase. Whether you choose the css prop or `styled` is a matter of personal preference. (If you're curious, the css prop among the maintainers of Emotion.)
...
Don't repeat yourself! If you are using a color, amount of padding, border radius, .etc throughout your application, add it to your theme or define it as a JavaScript constant, like
...
If you are writing a component library, carefully consider whether your library will depend on Emotion. A simple alternative is to include a regular CSS file in your package, which your users can then import via a normal `import` statement.
...
- Emotion may be more error prone than a simple CSS file in certain edge cases. In a previous version of [react-loading-skeleton](https://github.com/dvtng/react-loading-skeleton) which utilized Emotion 10, we received reports of styles not working in production environments, iframes, and the shadow DOM.

...
Server side rendering in emotion 10 has two approaches, each with their pros and cons. The default usage works with streaming and requires no additional configuration, but has some limitations on some CSS features.
...
You can also use the advanced integration, it requires more work but does not have the CSS limitations of the first approach. This approach does not work with the streaming APIs though.
...
<head>
    ...
    <title>My site</title>
    <style type="text/css" data-emotion-css="${ids.join(' ')}">${ssrRenderResult.css}</style>
</head>
<body>

...
The easiest way to use the css prop with TypeScript is with the new JSX
transform and the `jsxImportSource` TSConfig option. For this approach, your
TSConfig `compilerOptions` should contain
...
For most users, this is all the setup that is required. You can now define
styles using the object syntax or template literal syntax and pass them to your
components via the `css` prop.
...
[`@emotion/babel-plugin`](/docs/babel) is completely optional for TypeScript
users. If you not already using Babel, you probably shouldn't add it to your
build tooling unless you truly need one of the features offered by
`@emotion/babel-plugin`. On the other hand, there's no reason not to use
`@emotion/babel-plugin` if you are already using Babel to transpile your
TypeScript code.

...
If you are using the compatible React version (`>=17.0.1`) then you can opt into using [the new JSX runtimes](https://reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html) by using such configuration:
...
```json
{
  "presets": [
    [
      "@babel/preset-react",
      { "runtime": "automatic", "importSource": "@zedvision/emotion-react" }
    ]
  ],
  "plugins": ["@zedvision/emotion-babel-plugin"]
}
```
  ...
  "presets": [
    [
      "next/babel",
      {
        "preset-react": {
          "runtime": "automatic",
          "importSource": "@zedvision/emotion-react"
        }
      }
    ]
  ],

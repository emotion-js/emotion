...
```json
{
  "env": {
    "production": {
      "plugins": [["emotion", { "sourceMap": false, "hoist": true, "meta": false }]]
    },
    "development": {
      "plugins": [["emotion", { "sourceMap": true, "hoist": false, "meta": true  }]]
    }
  }
}
```
...
This option enables the following:
 ...
 - Class names generated by `css` or `styled` include the name of the variable the result is assigned to.  
...
const brownStyles = css(
  { color: 'brown' }, 
  { meta: { identifierName: 'brownStyles' }}
);
```

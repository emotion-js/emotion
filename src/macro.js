...
module.exports = function macro ({ references, state, babel: { types: t } }) {
  ...
  if (references.injectGlobal) {
    references.injectGlobal.forEach((injectGlobalReference) => {
      ...
      if (t.isIdentifier(path.node.tag) && t.isTemplateLiteral(path.node.quasi)) {
        replaceGlobalWithCallExpression(buildMacroRuntimeNode(injectGlobalReference, state, 'injectGlobal', t), injectGlobal, path, state, t)
      }
    })
  }
  if (references.fontFace) {
    references.fontFace.forEach((fontFaceReference) => {
      ...
      if (t.isIdentifier(path.node.tag) && t.isTemplateLiteral(path.node.quasi)) {
        replaceGlobalWithCallExpression(buildMacroRuntimeNode(fontFaceReference, state, 'fontFace', t), fontFace, path, state, t)
      }
    })
  }
  if (references.css) {
  ...
  if (references.keyframes) {
    references.keyframes.forEach((keyframesReference) => {
      ...
      if (t.isIdentifier(path.node.tag) && t.isTemplateLiteral(path.node.quasi)) {
        replaceKeyframesWithCallExpression(path, buildMacroRuntimeNode(keyframesReference, state, 'keyframes', t), state, t)
      }
    })
  }
  ['inserted', 'sheet', 'flush', 'hydrate'].forEach((identifier) => {
    if (references[identifier]) {
      references[identifier].forEach((reference) => {
        reference.replaceWith(buildMacroRuntimeNode(reference, state, identifier, t))
      })
    }
  })
